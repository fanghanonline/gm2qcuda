                              ...
                              float *SimulationParameters)
{
  ...
  if (idx < nFlushesPerBatch * nFillsPerFlush)
  {
    ...
    // muon decay
    while (nhit < NElectronsPerFill)
    { // should randomize the hits per fill
    ...
    { // loop over hits
      ...
      for (int ix = 0; ix < nxseg; ix++)
      {
        for (int iy = 0; iy < nyseg;
             iy++)
        { // calc energy in segment (assume aGaussian distribution
          // about xc, yc)
...
// Editor: Fang Han
// This version of make_randfill_fh function remove the flushHitArray(T-Method)
__global__ void make_randfill_fh(curandState *state, float *pulseTemplate,
                                 float *pedestalTemplate,
                                 float *flushQTruthArray,
                                 float *flushQArray,
                                 int *SimulationIntParameters,
                                 float *SimulationParameters)
{
  ...
  if (idx < nFlushesPerBatch * nFillsPerFlush)
  {
    ...
    // Add pedestal from the template
    for (int ix = 0; ix < nxseg; ix++)
    {
      for (int iy = 0; iy < nyseg;
           iy++)
...
namespace QSimulation
{
  ...
  int QSim::Simulate(int NFlushes)
  {
    ...
    for (int i = 0; i < NSim; i++)
    {
      ...
      make_randfill_fh<<<nblocks, IntParameters["NThreadsPerBlock"]>>>(
          d_state, d_IntegratedPulseTemplate, d_PedestalTemplate,
          DeviceArrays["FlushQTruthArray"], DeviceArrays["FlushQArray"],
          d_SimulationIntParameters, d_SimulationParameters);
